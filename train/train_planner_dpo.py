...
def preprocess_data(
    ...
    if apply_chat_template:
        if prompt_key:
            ...
            chosen = apply_chat_template(
                data[prompt_key] + data[chosen_key], tokenize=False
            )[len(prompt) :]
            rejected = apply_chat_template(
                data[prompt_key] + data[rejected_key], tokenize=False
            )[len(prompt) :]
        else:
            prompt = ""
            ...
            if is_dpo:
                # worker id is different in chosen and rejected, but we still use one prompt as the input
                prompt = apply_chat_template(
                    data[chosen_key][:-1], tokenize=False, add_generation_prompt=True
                )
                rejected_prompt = apply_chat_template(
                    data[rejected_key][:-1], tokenize=False, add_generation_prompt=True
                )
                chosen = chosen[len(prompt) :]
                rejected = rejected[len(rejected_prompt) :]
    else:
        if prompt_key:
...
class MyRewardDataset(RewardDataset):
...
def train(args):
    ...
    # strategy prepare
    ((model, optim, scheduler), ref_model) = strategy.prepare(
        (model, optim, scheduler), ref_model
    )
...
if __name__ == "__main__":
    ...
    if args.ring_attn_size > 1:
        assert (
            args.packing_samples
        ), "packing_samples must be enabled when using ring attention"
