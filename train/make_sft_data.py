...
def run(args):
    if args.dataset.endswith(".json"):
        with open(f"dataset/{args.dataset}", "r") as f:
            data = json.load(f)
    ...
    for item in data:
        kwargs = {
            ...
            "ground_truth": item["ground_truth"] if "ground_truth" in item else None,
        }
        ...
        for each_traj in trajectory:
            ...
            if len(each_traj["trajectory"]) != 1:
                print(
                    "detect multi-turn trajectory, for now we only select the last trajectory"
                )
            if each_traj["success"]:
                planner_history = subtasks["planner_history"]
                if len(planner_history) != 5:
                    print("wrong planner history")
                    # continue
                if (
                    planner_history[0]["role"] == "user"
                    and planner_history[1]["role"] == "system"
                ):
                    planner_history[0]["role"] = "system"
                    tmp = planner_history[0]["content"]
                    planner_history[0]["content"] = planner_history[1]["content"]
                    planner_history[1]["content"] = tmp
                    planner_history[1]["role"] = "user"
                for each_item in planner_history:
                    if each_item["role"] == "assistant" and "tool_calls" in each_item:
                        raise ValueError(
                            "we should not have tool calls in planner history"
                        )
                    if each_item["role"] == "tool":
                        raise ValueError(
                            "we should not have tool calls in planner history"
                        )
                final_data.extend(
            ...
            for i, each_subtask in enumerate(subtasks_history):
                ...
                final_data.append(
                    {
                        "conversations": new_subtask_history,
                        "tools": tools,
                        "type": "subtask",
                        "weight": (1 + (i + 1) / n) / 2,
                        **kwargs,
    ...
    with open(f"dataset/sft/{args.dataset}", "w") as f:
        json.dump(final_data, f, indent=4)
    print(f"Number of samples: {len(final_data)}")
    # push to hub
    data = Dataset.from_list(final_data)
    data.push_to_hub(f"owl-agent/{args.dataset.replace(".json","")}", private=True)
